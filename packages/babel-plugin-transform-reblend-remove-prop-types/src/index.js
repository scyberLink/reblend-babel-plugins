...
function isPathReblendClass(path, globalOptions) {
  const node = path.node
  const matchers = globalOptions.classNameMatchers

  if (path.matchesPattern('Reblend.Component') || path.matchesPattern('Reblend.PureComponent')) {
    return true
  }
...
function isReblendClass(superClass, scope, globalOptions) {
  if (!superClass.node) {
    return false
  }
  ...
  if (isPathReblendClass(superClass, globalOptions)) {
    answer = true
    ...
    if (!binding) {
      ...
      if (isPathReblendClass(bindingSuperClass, globalOptions)) {
        answer = true
      }
    }
  }
  ...
  return {
    visitor: {
      Program(programPath, state) {
        ...
        const globalOptions = {
          visitedKey: `transform-reblend-remove-prop-types${Date.now()}`,
          ...
          classNameMatchers,
          createReblendClassName: state.opts.createReblendClassName || 'createReblendClass',
        }
        ...
        programPath.traverse({
          ObjectProperty: {
            exit(path) {
              ...
              const parent = path.findParent((currentNode) => {
                ...
                return (
                  currentNode.get('callee').node.name === globalOptions.createReblendClassName ||
          ...
          ClassProperty(path) {
            ...
            if (node.key.name === 'propTypes') {
              ...
              if (isReblendClass(pathClassDeclaration.get('superClass'), scope, globalOptions)) {
                path.traverse(collectNestedIdentifiers)
                removedPaths.add(path)
                remove(path, globalOptions, {
                  type: 'class static',
          ...
          AssignmentExpression(path) {
            ...
            if (binding.path.isClassDeclaration()) {
              ...
              if (isReblendClass(superClass, scope, globalOptions)) {
                path.traverse(collectNestedIdentifiers)
                removedPaths.add(path)
                remove(path, globalOptions, { type: 'assign' })
              }
        ...
        if (globalOptions.removeImport) {
          if (globalOptions.mode === 'remove') {
            ...
            throw new Error(
              'transform-reblend-remove-prop-type: removeImport = true and mode != "remove" can not be used at the same time.'
            )
          }
        }
      },
