...
exports.default = (0, helper_plugin_utils_1.declare)(api => {
    ...
    function hasRefOrSpread(attrs) {
        for (let i = 0; i < attrs.length; i++) {
            const attr = attrs[i];
            if (core_1.types.isJSXSpreadAttribute(attr))
                return true;
    ...
    function isJSXAttributeOfName(attr, name) {
        return (core_1.types.isJSXAttribute(attr) && core_1.types.isJSXIdentifier(attr.name, { name: name }));
    }
    const visitor = (0, helper_builder_reblend_jsx_1.default)({
        filter(node) {
        ...
        post(state, pass) {
            ...
            if (!hasKey && state.args.length > 2) {
                state.args.splice(2, 0, core_1.types.unaryExpression('void', core_1.types.numericLiteral(0)));
            }
            state.pure = true;
        },
    });
